...
func get_adjacents(block_pos: Vector3) -> Array[Vector3i]:
	var adjacent_dirs = [Vector3i.UP, Vector3i.DOWN, Vector3i.LEFT, Vector3i.RIGHT, Vector3i.FORWARD, Vector3i.BACK]
	var adjacents: Array[Vector3i] = []
	var grid_block_pos: Vector3i = BlockGlobals.to_grid(block_pos)

	for adjacent_dir in adjacent_dirs:
		var adjacent_pos:  Vector3i = adjacent_dir * BlockGlobals.BLOCK_SIZE + grid_block_pos
...
func update_neighbors(block_pos: Vector3, block_grid: Node3D):
	...
	for adjacent in adjacents:
		var adjacent_block_inst: Node3D = BlockGlobals.blocks_dict[adjacent]
...
func update_block_and_neighbors(block_pos: Vector3, block_grid: Node3D):
	...
	update_block(BlockGlobals.blocks_dict[BlockGlobals.to_grid(block_pos)], block_grid)
